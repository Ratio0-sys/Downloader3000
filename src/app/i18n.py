...
def _locales_dir() -> Path:
    ...
    Берём именно `resource_dir()`, а не `app_dir()`: под PyInstaller
    ресурсы распакованы во временную папку, и рядом с exe их нет.
    Остальные варианты оставлены запасными — разные сборщики
    раскладывают файлы по-разному.
    """
    candidates = [
        pp.resource_dir() / "assets" / "locales",
