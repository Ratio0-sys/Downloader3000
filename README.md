...
**На Android возможности урезаны, и заметно.** Там недоступны сразу две вещи:
ffmpeg и движок JavaScript. Без ffmpeg не склеить видео со звуком и не сделать
MP3. Без движка JavaScript YouTube отдаёт неполный список форматов.
Вшить QuickJS в APK не выходит: он собирается под платформу сборщика,
а Android нужен свой бинарник. Программа сама определяет ситуацию
и показывает жёлтый статус с перечнем того, чего не хватает.
Скачивание с сайтов попроще при этом работает.
